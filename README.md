...
This project is an admin dashboard mockup created as part of a job application to [https://https://evertry.co/](EverTry). The task was to implement a dashboard that allows an admin to log in, view and manage appointments. New appointments can be added to the dashboard via an "add-to-list" button. The appointments can then be scheduled or cancelled depending on their current status. Appointments can also be deleted from the list.
